...
def main():
    ...
    try:
        r = requests.post(
            ...
            data="{\"vzRsSubjGraphAtt\":{\"attributes\":{\"dn\":\"uni/tn-SiriusCyber/brc-out-to-app1/subj-Subject/rsSubjGraphAtt\",\"status\":\"deleted\"},\"children\":[]}}",
            cookies = login_cookies
        )
    except requests.exceptions.RequestException as e:
        print('Remove FE SG HTTP Request failed')

    # Move the ISR L3out to the internal VRF
    try:
        r = requests.post(
            url="http://192.168.0.50/api/node/mo/uni/tn-SiriusCyber/out-ToISR/rsectx.json",
            data="{\"l3extRsEctx\":{\"attributes\":{\"tnFvCtxName\":\"Sirius-internal\"},\"children\":[]}}",
            cookies = login_cookies
        )
    except requests.exceptions.RequestException as e:
        print('Move ISR L3out to the internal VRF HTTP Request failed')
